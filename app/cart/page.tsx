      ...
      {cart.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          {cart.map((item, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => updateQty(item.id, -1)} className="w-8 h-8 rounded-lg bg-white/5 text-white hover:bg-white/15 transition-all duration-200 hover:scale-110">−</button>
                <span className="text-white w-6 text-center font-medium">{item.quantity}</span>
                <button onClick={() => updateQty(item.id, 1)} className="w-8 h-8 rounded-lg bg-white/5 text-white hover:bg-white/15 transition-all duration-200 hover:scale-110">+</button>
              </div>
              <button onClick={() => remove(item.id)} className="text-red-400/50 hover:text-red-400 transition-all duration-200 text-sm ml-2">✕</button>
            </div>
          ))}
          ...
          <div className="glass p-6 mt-2 animate-fade-in-up delay-300">
            ...
            <button onClick={checkout} className="btn-primary w-full flex items-center justify-center gap-2 text-base">
              <span>Checkout via WhatsApp</span>
              <span>→</span>
            </button>
            <p className="text-white/25 text-xs text-center mt-3">You&apos;ll be redirected to WhatsApp to confirm your order.</p>
          </div>
        </div>
      )}
    </div>
